...
void MuMuTau::eventLoop() 
{
  ...
  // Analysis is over
  nEvtMuMuTau[0] = nEvt;
  nEvtMuMuTau[1] = nEvtVtx;

  endJob();
}
void MuMuTau::findGenInfo() {
...
void MuMuTau::computeDeltaR(const vector<Muon>& muoList, const vector<Tau>& tauList ){
//findTauInfo();
//findMuoInfo();
...
*/
void MuMuTau::selectMuMuTauEvent(const vector<Muon>& muoList, 
...
void MuMuTau::computeSignificance(const vector<Muon>& muoList, 
                                  const vector<Tau>& tauList, 
                                  const vector<Jet>& bjetList) 
{
  // 2 muons
...
// -------------------------------------------------------------------------------
bool MuMuTau::readJob(const string& jobFile, int& nFiles)
{
...
void MuMuTau::printJob(ostream& os)
{
  AnaBase::printJob(os);
...
// -------------------------------------------------------------------------
TH1* MuMuTau::getHist1D(const string& hname) {
...
template <class T>
bool MuMuTau::fillHist1D(const string& hname, T value, double w) 
{
...
bool MuMuTau::fillHist2D(const string& hname, T1 xvalue, T2 yvalue, double w) 
{
  TObject *obj = gDirectory->GetList()->FindObject(hname.c_str()); 
  TH2 *h = 0;
...
// --------------------------------------------------
TProfile* MuMuTau::getProfile(const string& hname) 
{
...
bool MuMuTau::fillProfile(const string& hname, float xvalue, float yvalue, double w) 
{
