...
{
  new TH1D("counter_gen", "Selected event counter", 51, -0.5, 50.5);
  new TH1D("eventWeight", "Event Weight", 500001, -250000.5, 250000.5);
  new TH1F("npu", "nPU distribution", 50, 0, 50);
  new TH1F("ntrueint", "nTrueInt distribution", 50, 0, 50);
  new TH1F("nPV", "nPV distribution", 50, 0, 50);
  new TH1F("nPVCorrected", "nPV distribution", 50, 0, 50);
  ...
  new TH1F("mupt_mu", "mupt after muon", 40, 0, 200);
  new TH1F("mueta_mu", "mueta after muon", 100, -2.5, 2.5);
  new TH1F("tau1pt_tau1", "tau1pt after tau1", 40, 0, 200);
...
void SelectionEfficiency::eventLoop() 
{
  ...
  for (int ev = 0; ev < nEvents(); ++ev) {
    ...
    if (isMC()) {
      ...
      AnaUtil::fillHist1D("eventWeight", genevt.weightevt, puevWt_);
      puevWt_ *= genevt.weightevt;
    }
    ...
    //if (!WmHhhFilter()) continue;                                        //Put this off if not required::ATTENTION
    ...
    if (isMC()) {
      AnaUtil::fillHist1D("npu", evt.nPU.at(0), 1);
      AnaUtil::fillHist1D("ntrueint", evt.trueNInt.at(0), 1);
    }
...
void SelectionEfficiency::studyTauID(){
...
#if 0
void SelectionEfficiency::studyID(){
  vector<Muon> sigMuoList;
  vector<Muon> bkgMuoList;
...
void SelectionEfficiency::study_eff()
{
  ...
  for (auto it = muonColl()->begin(); it != muonColl()->end(); ++it) {
    ...
    //if (fabs(muon.vtxDistZ) >= AnaUtil::cutValue(muonCutMap(), "vtxDistZ")) continue;
    bool isGoodVtx;
    TVector3 vmu = findLeptonVtx(muon.vtxIndex, isGoodVtx);
    double dz = vmu.z() - vz;
  ...
  for (auto it = tauColl()->begin(); it != tauColl()->end(); ++it) {
    ...
    if (tau.byLooseCombinedIsolationDeltaBetaCorr3Hits > 0.5) continue;  //inverted for skim                       
  ...
  for (auto it = tauColl()->begin(); it != tauColl()->end(); ++it) {
    ...
    if (tau.pt <= AnaUtil::cutValue(_tau2CutMap, "pt") || 
      fabs(tau.eta) >= AnaUtil::cutValue(_tau2CutMap, "eta")) continue;
    ++t2counters[2];

    if (tau.decayModeFinding <= 0.5) continue;
    ++t2counters[3];
  
    //if (tau.chargedIsoPtSum >= 1.0) continue;                        
    if (tau.byMediumCombinedIsolationDeltaBetaCorr3Hits > 0.5) continue;  //inverted for skim                          
    ++t2counters[4];

    if (tau.againstMuonTight3 <= 0.5) continue; 
    ++t2counters[5];

    if (tau.againstElectronMediumMVA5 <= 0.5) continue;                         
    ++t2counters[6];

    if (fabs(tau.zvertex - vz) >= AnaUtil::cutValue(_tau2CutMap, "dz")) continue;   
    ++t2counters[7];
  ...
  //if (mass1 <= 30) return;  //Actual cut is 30
...
*/
  if (_skimObj) {
    ...
    varList.lepEta      = M.Eta();
    varList.lepPt       = M.Pt();
    varList.tauOSEta    = T1.Eta();
    varList.tauOSPt     = T1.Pt();
    varList.tauSSEta    = T2.Eta();
    varList.tauSSPt     = T2.Pt();
    varList.diTaudEta   = AnaUtil::deltaEta(T1, T2);
    varList.diTaudPhi   = AnaUtil::deltaPhi(T1, T2);
    varList.diTauPt    = diTau.Pt();
    varList.diTauMass    = diTau.M();
    varList.dphilepTauOS = AnaUtil::deltaPhi(M, T1);
    varList.dphilepTauSS = AnaUtil::deltaPhi(M, T2);
    varList.met        = pfmet.met;
    ...
    varList.alphalditau  = Mu_Alpha.Angle(d1+d2);
    varList.alphalprod   = Mu_Alpha.Angle(ditauCrossProd);
    double dphi = AnaUtil::deltaPhi(M.Phi(), pfmet.metphi); // acoplanarity
    varList.acop        = dphi;
    varList.dphilepDiTau = AnaUtil::deltaPhi(M, diTau);
    varList.DeltaRDiTau   = AnaUtil::deltaR(T1, T2);
