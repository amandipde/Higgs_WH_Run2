...
void MuonEfficiency::bookHistograms() 
{
  ...
  new TH1F("test_probePt_Iso_deno", "", 140, 0, 140);
  new TH1F("test_probePt_Iso_nume", "", 140, 0, 140);
  new TH1F("test_probeEta_nume_Med", "Medium WP", 100, -2.5, 2.5);
  new TH1F("test_probeEta_nume_Tig", "Tight WP", 100, -2.5, 2.5);
  new TH1F("test_probeEta_deno", "", 100, -2.5, 2.5);
  new TH1F("test_probePt_nume_trigger", "", 140, 0, 140);
  new TH1F("test_probePU_deno", "", 50, 0, 50);
  new TH1F("test_probePU_nume_trigger", "", 50, 0, 50);
  new TH1F("test_probePU_nume_Med", "Medium WP", 50, 0, 50);
  new TH1F("test_probePU_nume_Tig", "Tight WP", 50, 0, 50);
  new TH1F("test_probePU_Iso_deno", "", 50, 0, 50);
  new TH1F("test_probePU_Iso_nume", "", 50, 0, 50);
...
void MuonEfficiency::findMuonIDInfo() {
  ...
  for (vector<Muon>::const_iterator it  = preTagMuonList.begin(); 
    ...
    if (fabs(muon.dB) >= AnaUtil::cutValue(muonCutMap(),"dB")) continue;
    ...
    int tindx = -1;
    std::string tagPath = "HLT_IsoMu20";
    double drTag = matchTriggerObject(trigObjList, taglv, tagPath , -1, maxPtDiff, tindx);
#if 0
    if (0) cout << "=> indx = " << indx
                << " tindx = " << tindx
                << " drTag = " << drTag
    ...
    if (drTag >= AnaUtil::cutValue(_evselCutMap, "maxDr")) continue;
    tagindex = indx;
    tagTrigObjindex = tindx;
  ...
  double massMuMt = sqrt(2*tag.Pt()*Met.met*(1-cos(AnaUtil::deltaPhi(tag.Phi(), Met.metphi))));
  if (massMuMt > 40.0) return;
...
void MuonEfficiency::computeMuonEff() {
  ...
  if (fabs(prbeta) < 2.1 && prbpt > 5.0) {
    AnaUtil::fillHist1D("test_probePt_deno", prbpt, puevWt_);
    if (prbpt > 24.0){
      AnaUtil::fillHist1D("test_probeEta_deno", prbeta, puevWt_);
      AnaUtil::fillHist1D("test_probePU_deno", nvtx, puevWt_);
      if (pmuon.isTightMuon) AnaUtil::fillHist1D("test_probePU_Iso_deno", nvtx, puevWt_);
    }
    if (pmuon.isTightMuon) AnaUtil::fillHist1D("test_probePt_Iso_deno", prbpt, puevWt_);
  }
  if (pmuon.isMediumMuon && fabs(prbeta) < 2.1 && prbpt > 5.0){
    AnaUtil::fillHist1D("test_probePt_nume_Med", prbpt, puevWt_);
    if (prbpt > 24.0){
      AnaUtil::fillHist1D("test_probeEta_nume_Med", prbeta, puevWt_);
      AnaUtil::fillHist1D("test_probePU_nume_Med", nvtx, puevWt_);
    }
  }
  if (fabs(prbeta) < 2.1 && pmuon.isTightMuon && prbpt > 5.0){ 
   AnaUtil::fillHist1D("test_probePt_nume_Tig", prbpt, puevWt_);
   if (prbpt > 24.0){
     AnaUtil::fillHist1D("test_probeEta_nume_Tig", prbeta, puevWt_);
     AnaUtil::fillHist1D("test_probePU_nume_Tig", nvtx, puevWt_);
     if (fabs(pmuon.pfRelIsoDB03) < AnaUtil::cutValue(muonCutMap(), "pfRelIso")) AnaUtil::fillHist1D("test_probePU_Iso_nume", nvtx, puevWt_);
   }
   if (fabs(pmuon.pfRelIsoDB03) < AnaUtil::cutValue(muonCutMap(), "pfRelIso")) AnaUtil::fillHist1D("test_probePt_Iso_nume", prbpt, puevWt_);
  }

  double maxPtDiff = AnaUtil::cutValue(_evselCutMap, "maxPtDiff");
  std::string tagPath = "HLT_IsoMu20";
  int tindx = -1;
  double drProbe = matchTriggerObject(trigObjList, p, tagPath , tagTrigObjindex, maxPtDiff, tindx);
  if (drProbe < AnaUtil::cutValue(_evselCutMap, "maxDr") && fabs (prbeta) < 2.1 && prbpt > 5.0){ 
     AnaUtil::fillHist1D("test_probePt_nume_trigger", prbpt, puevWt_);
     if (prbpt > 24.0) AnaUtil::fillHist1D("test_probePU_nume_trigger", nvtx, puevWt_);
  }

#if 0
//double mass = (t+p).M();
//if (fabs(prbeta) < 1.44) AnaUtil::fillHist1D("mupt_probe_before_low", prbpt, puevWt_);
//else if (1.44 <= fabs(prbeta) && fabs(prbeta) < 2.1) AnaUtil::fillHist1D("mupt_probe_before_high", prbpt, puevWt_);
