...
void MuonTriggerEfficiency::selectEvent() 
{
  ...
  if (_dumpEvent) {
    dumpTriggerPaths(fLog());
    dumpEvent("01010", fLog(), true);
    dumpTriggerObjectInfo(trigObjVec, fLog());
  }
...
void MuonTriggerEfficiency::endJob() 
{
  ...
  histo_map.insert(std::make_pair(_probePtv, _matchedProbePtv));
  histo_map.insert(std::make_pair(_probeEta, _matchedProbeEta));
  histo_map.insert(std::make_pair(_probePhi, _matchedProbePhi));
...
void MuonTriggerEfficiency::fillTriggerPrescaleHistograms(int run, int lumi) {
  ...
  for (uint i = 0; i < hltpaths()->size(); i++) {
    ...
    int prescale     = hltprescales()->at(i);  
    ...
    for (vector<string>::iterator it  = trigPathList_.begin(); 
                                  it != trigPathList_.end(); ++it) 
    {
	...
	if (iPos != _triggerHistoMap.end()) {
	  ...
	  tp->Fill(run, prescale);
	    ...
	    if (muonVec.size() > 0 )  iPos->second.firstMuPtAcc->Fill(muonVec.at(0).pt, puevWt_);
	    if (muonVec.size() > 1 )  iPos->second.secondMuPtAcc->Fill(muonVec.at(1).pt, puevWt_);
          }
	}      
      }
    }
  ...
  for (uint iobj = 0; iobj < muonVec.size(); ++iobj) {
   ...
   // cout<<dRTag<<"   tag->"<<tag_trig_indx<<endl;
    ...
    if (dRTag <= AnaUtil::cutValue(_evselCutMap, "maxDr") && _iFlagL1 == 1) {
      _tagPt->Fill(tagv.Pt(), puevWt_);
      _tagPtv->Fill(tagv.Pt(), puevWt_);
      _tagEta->Fill(tagv.Eta(), puevWt_);
      _tagPhi->Fill(tagv.Phi(), puevWt_);
    
      TLorentzVector tagTrigTL;
      tagTrigTL.SetPtEtaPhiE(tagTrigObj.pt, tagTrigObj.eta, tagTrigObj.phi, tagTrigObj.energy);
      _trigTagPt->Fill(tagTrigObj.pt, puevWt_);
      _trigTagEta->Fill(tagTrigObj.eta, puevWt_);
      _trigTagPhi->Fill(tagTrigObj.phi, puevWt_);
      _tagVsTrigPt->Fill(tagv.Pt(), tagTrigObj.pt, puevWt_);
      _tagDeltaPt->Fill(dRPtTag, puevWt_);
      _tagVsTrigEta->Fill(tagv.Eta(), tagTrigObj.eta, puevWt_);
      _tagVsTrigPhi->Fill(tagv.Phi(), tagTrigObj.phi, puevWt_);
	...
	if (_iFlagL2 == 1) 
          _invMassPass->Fill(mass, puevWt_);
	else 
          _invMassFail->Fill(mass, puevWt_);
	...
	if (deltaM < AnaUtil::cutValue(_evselCutMap, "massWindowWide")) {
	  ...
	  probeTrigTL.SetPtEtaPhiE(probeTrigObj.pt, probeTrigObj.eta, probeTrigObj.phi, probeTrigObj.energy);
	    ...
	    _probePt->Fill(probev.Pt(), puevWt_);
	    _probePtv->Fill(probev.Pt(), puevWt_);
	    _probeEta->Fill(probev.Eta(), puevWt_);
	    _probePhi->Fill(probev.Phi(), puevWt_);
	    _probeDRVsInvMass->Fill(mass, dRProbe, puevWt_); 
	    _probeDeltaR->Fill(dRProbe, puevWt_);
	    _probeVsTagDR->Fill(dRTag, dRProbe, puevWt_);
	  }
	  ...
	  if (dRProbe <= AnaUtil::cutValue(_evselCutMap, "maxDr") && _iFlagL2 == 1) {
            if (deltaM < AnaUtil::cutValue(_evselCutMap, "massWindowNarrow")) {
	      _matchedProbePt->Fill(probev.Pt(), puevWt_);
	      _matchedProbePtv->Fill(probev.Pt(), puevWt_);
	      _matchedProbeEta->Fill(probev.Eta(), puevWt_);
	      _matchedProbePhi->Fill(probev.Phi(), puevWt_);
	      _probeVsTrigPt->Fill(probev.Pt(), probeTrigObj.pt, puevWt_);
	      _probeDeltaPt->Fill(dRPtProbe, puevWt_);
	      _probeVsTrigEta->Fill(probev.Eta(), probeTrigObj.eta, puevWt_);
	      _probeVsTrigPhi->Fill(probev.Phi(), probeTrigObj.phi, puevWt_);
	      _invMassTrigPass->Fill(massTrig, puevWt_);
	    }
	    _invMassVsPtPass->Fill(probev.Pt(), mass, puevWt_); 
	    break;
	  } 
          else {
	    if (deltaM < AnaUtil::cutValue(_evselCutMap, "massWindowNarrow")) {
	      _probeDeltaRFail->Fill(dRProbe, puevWt_);
	      _probeVsTagDRFail->Fill(dRTag, dRProbe, puevWt_);
	      _probeTrigDeltaPtFail->Fill(probev.Pt() - probeTrigObj.pt, puevWt_);
	      _probeVsTrigPtFail->Fill(probev.Pt(), probeTrigObj.pt, puevWt_);
	      _probeVsTrigEtaFail->Fill(probev.Eta(), probeTrigObj.eta, puevWt_);
	      _probeVsTrigPhiFail->Fill(probev.Phi(), probeTrigObj.phi, puevWt_);
	      _invMassTrigFail->Fill(massTrig, puevWt_);
	    }
	    _invMassVsPtFail->Fill(probev.Pt(), mass, puevWt_); 
          } 
	}
      }
...
void MuonTriggerEfficiency::fillTriggerEfficiencies(std::map<TH1F*, TH1F*> & histo_map) {
  ...
  for (map<string, TriggerHistos>::iterator it  = _triggerHistoMap.begin(); 
    ...
    fLog() << " Trigger Path " << tname << " events " <<  it->second.eventPassed << endl;
  ...
  for (std::map<TH1F*, TH1F*>::iterator it  = histo_map.begin(); 
    ...
    if (evt1 > 0.0 && hname1.find("Pt") != string::npos) {
      ...
      fLog() << " Probe Events " << evt1 << "  Matched Probe Events " << evt2 << endl;
      cout << " Average Efficiency " << av_eff << " +- " << av_eff_err << endl;
      fLog() << " Average Efficiency " << av_eff << " +- " << av_eff_err << endl;
    }
	...
	std::cout << j << " " << hist1->GetBinCenter(j) << " " << val1 << " " << val2 << " " << ratio << std::endl;
            fLog() << j << " " << hist1->GetBinCenter(j) << " " << val1 << " " << val2 << " " << ratio << std::endl;
      }
    }
  }
}
